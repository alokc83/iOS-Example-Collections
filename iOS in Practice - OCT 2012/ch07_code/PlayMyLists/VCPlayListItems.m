...
- (void)configureCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    NSManagedObject *managedObject = [[[(AppPlayList*)_detailItem tracks] allObjects] objectAtIndex:indexPath.row]; 
...
{
    NSString *pID = [NSString stringWithFormat:@"%@",
                     [mediaItem valueForProperty:MPMediaItemPropertyPersistentID]];
    if ([[[_detailItem tracks] allObjects] containsObject:pID])
        return;
    ...
    [newManagedObject setValue:pID forKey:@"persistentID"];
